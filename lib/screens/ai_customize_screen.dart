...
class _AICustomizeScreenState extends State<AICustomizeScreen>
  ...
  void _showQuickReplies() {
    final quickReplies = [
      // 🧋 Level 1：奶茶成瘾指数计算器 - 复杂健康评估
      '添加"奶茶成瘾度"按键，输入体重一键计算个人专属奶茶安全指数：糖分代谢率×体重×年龄系数×运动频率，预设60kg成人每日糖分上限25g',
      
      // 🎮 Level 2：游戏氪金性价比分析器 - 多维度消费评估  
      '新增"氪金性价比"按键，输入充值金额自动计算真实价值：充值金额÷(游戏时长×技能提升率×社交价值×时间成本)，预设复杂游戏经济学模型',
      
      // 😴 Level 3：熬夜生命损耗计算器 - 生物钟健康评估
      '增加"熬夜生命值"按键，输入年龄一键计算熬夜对寿命的影响：(24-睡眠时间)²×年龄系数×身体指数×恢复能力，科学量化熬夜成本',
      
      // 🏠 Level 4：一键房贷计算器 - 复杂金融建模
      '添加"房贷神器"按键，输入贷款金额自动计算月供：本金×[(利率3.5%÷12)×(1+利率÷12)^(30×12)]÷[(1+利率÷12)^(30×12)-1]，预设30年等额本息',
      
      // 🛍️ Level 5：网购真实成本分析器 - 隐性消费揭示
      '新增"网购真相"按键，输入商品价格自动计算真实成本：商品价×(1+快递费率+时间成本+退换货风险+冲动购买溢价)，揭示购物隐性成本',
      
      // 💪 Level 6：个人减肥效率计算器 - 科学健身建模
      '添加"减肥效率"按键，输入目标体重自动计算减肥方案：[(当前体重-目标体重)×7700卡路里]÷[(运动消耗+饮食控制)×基础代谢率×年龄系数]天',
      
      // ☕ Level 7：咖啡因代谢个性化计算器 - 精准健康管理
      '新增"咖啡因代谢"按键，输入体重自动计算个人咖啡因安全摄入量：体重×6mg×代谢率×年龄系数×健康指数，预设个体差异化计算',
      
      // 💕 Level 8：恋爱成本效益分析器 - 情感经济学
      '增加"恋爱成本"按键，输入月收入自动计算恋爱总成本：(约会费用+礼物开销+时间成本+机会成本)×感情投资回报率×幸福指数权重',
      
      // 🚗 Level 9：购车综合成本计算器 - 全生命周期分析
      '添加"购车真相"按键，输入车价自动计算5年总成本：车价+保险×5年+油费×里程+保养费×次数+折旧率×时间+停车费×使用频率',
      
      // 🍕 Level 10：外卖健康成本双重计算器 - 营养经济学
      '新增"外卖双重成本"按键，输入月外卖次数自动计算：经济成本(外卖费×次数×溢价率)+健康成本(营养缺失×身体负担×长期影响系数)',
      
      // 💼 Level 11：跳槽收益风险评估器 - 职业决策模型
      '增加"跳槽分析"按键，输入新工作薪资自动计算跳槽价值：(新薪资-旧薪资)×稳定系数×发展潜力×学习成长×行业前景-跳槽风险成本',
      
      // 🌟 Level 12：网红打卡投资回报率计算器 - 社交资本分析
      '添加"打卡ROI"按键，输入打卡费用自动计算社交投资回报：打卡成本÷(社交影响力×个人品牌价值×网络效应×长期收益潜力)，理性社交消费',
    ];
