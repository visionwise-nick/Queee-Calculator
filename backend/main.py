...
📐 **严格布局规则 - 屏幕适配优先**：
```
🚨 CRITICAL: 布局必须严格遵循以下限制
❌ 绝对禁止超出：6行×5列（最大30个按键）
❌ 推荐限制：5行×4列（最大20个按键）
❌ 按键总数：绝对不超过24个按键

标准计算器布局（强制推荐5行×4列）：
行1: [AC] [±] [%] [÷]      - 功能行 (4个按键)
行2: [7] [8] [9] [×]       - 数字+运算符 (4个按键)
行3: [4] [5] [6] [-]       - 数字+运算符 (4个按键)
行4: [1] [2] [3] [+]       - 数字+运算符 (4个按键)
行5: [0] [.] [=] [功能]     - 底行 (4个按键)
总计：20个按键 ✅ 完美适配所有屏幕

扩展科学计算器（最大6行×5列）：
行1-5: 标准布局 (20个按键)
行6: [sin] [cos] [tan] [log] [√] - 科学函数 (5个按键)
总计：25个按键 ⚠️ 仅在必要时使用

🚨 强制约束：
- 最大行数：6行 (row ≤ 6)
- 最大列数：5列 (column ≤ 4，因为从0开始)
- 最大按键数：24个按键
- 屏幕适配：确保在320px宽度设备上正常显示
- 按键尺寸：宽度倍数≤4，高度倍数≤3
- 位置检查：严禁按键位置冲突或重叠

🎯 布局优先级策略：
1. 基础功能优先：数字键(0-9)、运算符(+,-,×,÷,=)、清除(AC)
2. 常用功能其次：小数点(.)、正负号(±)、百分比(%)
3. 高级功能最后：科学函数、自定义功能
4. 如果空间不足，优先保留基础功能，删减高级功能

⚠️ 关键：严格控制按键数量，宁可功能简单也要确保显示正常！
⚠️ 禁止：为了功能丰富而牺牲屏幕适配性！
```
...
🎯 布局尺寸强制限制：
- 最大行数：6行 (row ≤ 6，超出自动压缩)
- 最大列数：5列 (column ≤ 4，超出自动压缩)
- 推荐尺寸：5行×4列 (最佳兼容性)

🎯 按键大小倍数强制修复：
- 宽度倍数：1-4之间的整数 (小数自动取整)
- 高度倍数：1-3之间的整数 (小数自动取整)
- 超出范围：自动限制到允许范围
...
"""
        ...
        try:
            ...
            # 🚨 严格布局约束验证
            final_config = enforce_strict_layout_constraints(final_config)
